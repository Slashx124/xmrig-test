...
func main() {
	...
	for iter := 0; iter < 280; iter++ {
		...
		if iter < 20 || (iter >= 255 && iter <= 265) {
			fmt.Printf("Iter %d: op=%d pos1=%d pos2=%d lhash=%016x prev=%016x\n", iter, op, pos1, pos2, lhash, prev_lhash)
			fmt.Printf("  s[0:16]=%s s[255]=%02x\n", hex.EncodeToString(step_3[:16]), step_3[255])
		}

		_ = step_3[pos1:pos2]

		// Execute the same switch as pow.go (just first few cases for brevity, but we need all 256)
		// Instead of duplicating, let's just track key values
		s := step_3[:]
		...
		default:
			// For a proper test we need all 256 cases - skip for now
			// Just break to see where divergence starts
		}

		// Post-loop hash updates (same as pow.go)
		if byte(s[pos1]-s[pos2]) < 0x10 {
			prev_lhash = lhash + prev_lhash
			lhash = xxhash.Sum64(s[:pos2])
		}
		if byte(s[pos1]-s[pos2]) < 0x20 {
			prev_lhash = lhash + prev_lhash
			lhash = fnv1a.HashBytes64(s[:pos2])
		}
		if byte(s[pos1]-s[pos2]) < 0x30 {
			prev_lhash = lhash + prev_lhash
			lhash = siphash.Hash(tries, prev_lhash, s[:pos2])
		}
		if byte(s[pos1]-s[pos2]) <= 0x40 {
