...
#   if defined(XMRIG_FEATURE_TLS) && defined(XMRIG_ALGO_ASTROBWT)
    if (m_pool.isWSS() && (m_apiVersion == API_DERO) && (m_pool.algorithm().id() == Algorithm::ASTROBWT_DERO_3)) {
        char buf[256];
...
#   ifdef XMRIG_ALGO_ASTROBWT
    const Algorithm algo = m_pool.algorithm();
    if ((algo.id() == Algorithm::ASTROBWT_DERO_3) || (m_coin == Coin::DERO_HE)) {
...
#   ifdef XMRIG_ALGO_ASTROBWT
    if (m_apiVersion == API_DERO) {
        // Truncate to 64 chars (32 bytes) to have the same data as in get_info RPC
...
#   ifdef XMRIG_ALGO_ASTROBWT
    if ((m_apiVersion == API_DERO) && result.HasMember("status") && result["status"].IsString()) {
...
void xmrig::DaemonClient::WSSRead(ssize_t nread, const uv_buf_t* read_buf)
{
    ...
    while ((n = SSL_read(m_wss.m_ssl, m_wss.m_buf, sizeof(m_wss.m_buf))) > 0) {
            ...
            if (len >= 4) {
                for (size_t k = 0; k <= len - 4; ++k) {
                    if (memcmp(m_wss.m_data.data() + k, "\r\n\r\n", 4) == 0) {
                        ...
                        m_wss.m_data.erase(m_wss.m_data.begin(), m_wss.m_data.begin() + static_cast<std::vector<char>::difference_type>(k + 4));
                        break;
                    }
                }
            }
        ...
        m_wss.m_data.erase(m_wss.m_data.begin(), m_wss.m_data.begin() + static_cast<std::vector<char>::difference_type>(p - p0));
